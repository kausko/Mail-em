...
export default async function handler(req, res) {
  ...
  await initMiddleware(multer().any())(req, res);
  ...
  CSVData.forEach(({ data }) => {
    messages.push({
      ...
      ...(
        data.type && ["cc", "bcc", "to"].some(t => t === data.type) ? 
        { [data.type]: data.email } 
        : 
        { to: data.email }
      ),
  ...
  transporter.on("idle", () => {
    while (transporter.isIdle() && messages.length) {
      transporter
        ...
        .then(() => {console.log(toSend--)})
        .catch(console.log)
    }
    if (!toSend)
      return transporter.removeAllListeners().close()
  })
